export function updateDistance_auto() {
...
export function validateRoutes_auto() {
...
export function promptRFIDInput_auto() {
    const fromRouteValue = document.getElementById('fromRoute').innerText;
    const toRouteValue = document.getElementById('toRoute').value;
    const distance = Math.abs(fromRoute.post - toRoute.post);
...
export function processPayment_auto(paymentType) {
    if (!validateRoutes()) {
        return;
    }
    if (paymentType === 'cash') {
